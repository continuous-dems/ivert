...
List the ICESat-2 photon classification codes and their meanings. These are the codes IVERT assigns to photons during classification and that you use when filtering photons — for example with the `--classes` option of [`ivert database download`](/user_guide/database.md#photon-class-options), [`ivert database convert`](/user_guide/database.md#filtering-options), and [`ivert validate`](/user_guide/validate.md#photon-filtering).
