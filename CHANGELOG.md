...
### Changed
- **Breaking:** `ivert database export` is renamed `ivert database convert`, since it converts photons into GIS formats rather than exporting the database as it is (which is now `dump`). `ivert database export` still works for now as an alias for `convert`, with the same options, but prints a deprecation warning; it will be removed in a later release.
