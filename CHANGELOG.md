...
### Changed
- The `Created <n> records` line that appeared whenever IVERT wrote a vector file — the GeoPackage or Shapefile error export from `ivert validate`, among others — no longer shows below debug verbosity. It came from pyogrio, the library `GeoDataFrame.to_file()` writes through, which logs it at `INFO` for every file; the `raw:` some consoles printed in front of it was pyogrio's module name, not part of IVERT's output. `logging_config` now sets the `pyogrio` logger to `WARNING` unless the verbosity is `debug`, in the CLI and in spawned validation workers alike, so pyogrio's warnings still come through.
