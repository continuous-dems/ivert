...
### Changed
- `ivert database rebuild` shows a progress bar while it reads the granule files' headers, when run in a terminal. The bar stays off when output is redirected to a file or a pipe, and log messages print above it rather than breaking it. The same bar appears when a query rebuilds a missing index on its own. The command now finishes with a single line, `Rebuilt ivert database index with <n> granules.` (`granule` when there is exactly one), instead of also logging `Created _ivert_database_index.nc with <n> records.`, which now shows only at debug level.
