...
- The `nsidc_atl_version` setting now actually selects the ATL03 version that gets downloaded (#110). It was documented in `ivert_defaults.ini` but nothing read it: IVERT was on v007 only because that is the default inside `fetchez`, so a `fetchez` upgrade could have moved IVERT to a newer ATL03 release on its own, before the ATL08 granules that classify its photons had caught up. The value is now passed to `fetchez`, and because `fetchez` falls back to whatever Harmony serves for plain `ATL03` when it does not recognise a version, every downloaded granule is also checked against the setting and ignored with an error if its release differs. The version is handed to `globato` too, which refuses an ATL03 granule of any other release. `7`, `007` and `"007"` are all accepted. ATL24 is unaffected: it has no v007 and is matched to ATL03 by photon time, not by release.
- Cached Harmony jobs are only re-used for the ATL version they were submitted for (#110). `requests.csv` matched on dataset and bounding box alone, so after a change of `nsidc_atl_version` an unexpired job for the old version would have been picked up again. The file gains an `atl_version` column; existing files are upgraded when next read, and their records, which have no version, are not re-used — at most one resubmitted job per region.

### Changed
- The minimum `globato` version is now 0.3.9 (#110), the first release with the `atl_version` argument to `globato.read()` (globato#236), through which IVERT hands `nsidc_atl_version` to the classifier. Older releases accept the argument but ignore it, so globato's own check that an ATL03 granule is the configured release does not happen; IVERT's check of each downloaded granule's release still applies.
