...
### Changed
- Any `-mcp`, `-mcpl` or `-mcpb` coverage threshold now turns on `-mc/--measure-coverage` by itself, instead of stopping with an error when `-mc` was not also given.
