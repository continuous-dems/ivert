...
### Added
- `ivert validate` takes separate minimum-coverage thresholds for land and bathymetry cells: `-mcpl/--minimum-coverage-pct-land` and `-mcpb/--minimum-coverage-pct-bathy` (#115). Each overrides `-mcp/--minimum-coverage-pct` for its type of cell, so the few, sparse bathymetry cells need not be held to a threshold that suits land. A bathymetry cell is one with any bathymetry photons, as in the plots and summary stats. `validate_dem()`, `validate_dem_parallel()` and `validate_list_of_dems()` gain `min_coverage_pct_land` and `min_coverage_pct_bathy` arguments to match, and the log now reports how many cells of each type remain after the filter.
- `ivert database dump` packs the database, or a region and date range of it, into one compressed zip archive, and `ivert database restore` unpacks such an archive into the local database and rebuilds the index, so a database can be moved between machines, backed up, or shared a region at a time (#116). The archive holds the photon granule files and the landmask tiles as they are, a `manifest.json`, and a `summary.txt` of what is inside (region and dates, granule and photon counts by class, vertical datum, landmask tiles), which both commands also print at `info` verbosity. A dump takes the same region arguments as `convert` (a bounding box, a raster's extent, or a vector file's polygons) plus `-ds`/`-de`, and clips granule files and landmask tiles that reach outside them, so the archive claims only the area and dates it holds. Where an archive overlaps data already in the database, `restore` lists the overlap and asks whether to import `all` of it (duplicating photons), `keep` the existing data and import only the rest, `replace` the existing data in the archive's area, or `cancel`; `-oo/--on-overlap` makes the choice up front, and is required when IVERT is not running in a terminal. `keep` and `replace` clip granule files to what remains once the other side's area and dates are cut out, so no photon is stored twice and none is lost. `--dry-run` shows the summary and the overlap without changing anything. An archive whose vertical datum differs from the database's is refused. Both commands show progress bars at `info` verbosity when run in a terminal: `dump` while it stages (and clips) granule files and landmask tiles and while it compresses, `restore` while it unpacks the archive, imports or cuts granule files and landmask tiles, and rebuilds the index; the compress and unpack bars count bytes, so they keep moving through a large file. `IS2Database` gains `clip_granule_file()`, which writes the photons of a granule file that fall in each of a set of (x, y, t) boxes to files of their own.
- `ivert validate` removes misclassified ATL24 bathy-floor (class 40) photons before validating (#114). ATL24 labels as seafloor some returns a metre or two below the sea surface over water hundreds of metres deep, and some noise tens of metres below sea level under dry land; in the CRM Vol 6 North validation these made the bathymetry side read −288 ± 553 m. Four filters, all on by default, check each class-40 photon against ETOPO 2022 and the OpenStreetMap landmask: `deep` (the shallowest ETOPO cell within 1000 m is deeper than 30 m), `offshore` (outside the landmask, less than 5 m below the EGM2008 geoid and more than 500 m from the coastline), `reference` (more than 30 m shallower than ETOPO) and `land` (inside the landmask and more than 10 m below the geoid). On the North data they remove 99.7% of the false offshore photons and 95.5% of the false inland ones, and 0.1% of the photons that agree with the CRM; estuary photons (Elkhorn Slough, Morro Bay) are kept. `-bf/--bathy-filters` picks the filters (`none` turns them off, reproducing earlier results exactly), each threshold has its own flag and `bathy_*` setting, and `--bathy-ref-raster` substitutes another reference for ETOPO. The rules, thresholds and per-filter counts go into the summary stats file and onto the results `.h5` as an attribute, so a summary regenerated later still lists them. Only class-40 photons are touched. The `offshore` filter measures depth below the geoid rather than the instantaneous sea surface, so it ignores the tide; #112 tracks storing the real sea surface per photon.
- IVERT keeps an OpenStreetMap landmask for each storage tile of the photon database in the new `ivert_landmask_directory` setting (default `~/.ivert/database/landmasks`), so validations do not query OpenStreetMap again for land the database download already fetched (#114). `ivert database download` stores each part's tiles from the landmask globato just cached; a validation fills in any tile still missing (for databases built before this change, or area past the database's edge in 1° cells), from the download cache where it covers the tile and from OpenStreetMap otherwise. The store lives with the database rather than in the cache, so clearing the cache does not lose it.
- `ivert database download` accepts polygon vector files (`.gpkg`, `.shp`, `.geojson`, `.json`, `.gml`, `.kml`) as its region, alongside a bounding box or DEM files (#105). Every file's footprint — a vector file's polygons, a DEM's extent — is dissolved into one region, and the download covers it with the fewest rectangular requests it can: the region's bounding box is cut into 1° squares, the squares the region does not reach are dropped, and the rest are merged back into rectangles with `cuboid_funcs.merge_cuboids()`, preferring north-south strips because ICESat-2 ground tracks run closer to north-south than east-west, so tall rectangles cut across fewer passes and each pass needs fewer granule subsets. A clipped edge row or column narrower than a quarter of a square is folded into its neighbour first, so no request is a thin sliver. A file holding hundreds of adjacent tile outlines is therefore fetched with a handful of requests rather than one per outline, and the gaps between scattered outlines are not fetched. `download_new_granules()` gains a `geometry` argument for this and a `filter_query_bboxes()` companion to `filter_query_bbox()` that subtracts the database's existing coverage from several boxes in one read; the 2° `split_big_bboxes` tiling is not applied on top of a geometry's rectangles. `merge_cuboids()` gains an optional `prefer` argument (`'row'`/`'r'` or `'column'`/`'c'`) that tries both row-major and column-major merge orders and breaks a tie in favour of the named one; left out, it merges in the given order as before. Vector files are read in their own coordinate system and reprojected to WGS84 (a file with no CRS is assumed to be WGS84, with a warning); `-p` applies only to a numeric bounding box.

### Fixed
- The combined results of a collection validation now carry the right DEM filename on each row (#113). `validate_list_of_dems()` named the results of the i-th DEM that produced any after the i-th DEM in the whole list, but DEMs that come back empty or fail produce no results file, so after the first of them every row was labelled with the name of an earlier DEM. The cell values themselves were right; everything that groups by filename was not — the per-DEM rows of the `_individual_results.csv` summary, and the `filename` column of the combined `_results.h5` — and results could be listed under DEMs that had none. In a 169-tile run with 79 empty tiles, 88% of the combined rows were mislabelled. The per-DEM `_results.h5` files were never affected.
- The `along_track_m` column is written to the database tiles again (#108). Each photon's cumulative along-track distance was meant to be taken from the `.h5` and merged onto the classified photons, but the beam name globato hands over is 4 bytes (`b'gt1l'`), the HDF5 path was built from its `repr`, and the lookup failed quietly for every beam, so no tile had the column and `plot_photon_clouds_v2.py`, which reads it when no `.h5` is given, skipped every beam. The distance is now looked up by the photon's row in the file (its segment and place within it, which globato carries through) rather than by matching coordinates, so ATL24 bathymetry photons, whose coordinates globato replaces with ATL24's, get their distance too, and the join no longer costs a merge over every photon of the file. The formula that computes the distance from the `.h5`, shared with `plot_photon_clouds_v2.py`, also matched a photon's 0-based row against the 1-based `ph_index_beg`, which put the first photon of every segment (about 1% of photons) one segment too early, and it broke down on beams with empty segments, whose `ph_index_beg` is 0; both are corrected.
- Photons along the boundaries between the parts of a multi-part download are no longer lost (#106). Harmony names every subset of a granule alike whatever box it was cut to, fetchez keeps any file that already exists, and adjacent parts of a request share every granule whose ground track crosses their common edge, so the second part read the first part's subset, clipped it to its own box, and got nothing from those granules. Downloaded subsets are now named after the granule and the box they were cut to, the same suffix the database's `.nc` files carry, so each part fetches and reads its own. Existing cache files keep their old names and are simply not reused.
...
### Changed
- Any `-mcp`, `-mcpl` or `-mcpb` coverage threshold now turns on `-mc/--measure-coverage` by itself, instead of stopping with an error when `-mc` was not also given (#115).
- **Breaking:** `ivert database export` is renamed `ivert database convert`, since it converts photons into GIS formats rather than exporting the database as it is (which is now `dump`) (#116). `ivert database export` still works for now as an alias for `convert`, with the same options, but prints a deprecation warning; it will be removed in a later release.
- The `Created <n> records` line that appeared whenever IVERT wrote a vector file — the GeoPackage or Shapefile error export from `ivert validate`, among others — no longer shows below debug verbosity (#111). It came from pyogrio, the library `GeoDataFrame.to_file()` writes through, which logs it at `INFO` for every file; the `raw:` some consoles printed in front of it was pyogrio's module name, not part of IVERT's output. `logging_config` now sets the `pyogrio` logger to `WARNING` unless the verbosity is `debug`, in the CLI and in spawned validation workers alike, so pyogrio's warnings still come through.
- `ivert database rebuild` shows a progress bar while it reads the granule files' headers, when run in a terminal (#109). The bar stays off when output is redirected to a file or a pipe, and log messages print above it rather than breaking it. The same bar appears when a query rebuilds a missing index on its own. The command now finishes with a single line, `Rebuilt ivert database index with <n> granules.` (`granule` when there is exactly one), instead of also logging `Created _ivert_database_index.nc with <n> records.`, which now shows only at debug level.
- `ivert database download` classifies the granules of a request in parallel where the machine allows (#108). The largest subset is classified in the main process, then the rest go to a pool of forked worker processes sized at run time by the new `icesat2_classify_workers` setting. `auto`, the default, uses one fewer than the machine's cores, no more than the memory currently available allows at about six times the size of the largest subset file per process, and at most 8 — so a laptop with a few gigabytes free runs one or two workers while a large server runs eight; an integer forces that many, and `1` is the previous one-granule-at-a-time path. Workers run at a lower priority and are only forked on Linux; elsewhere `auto` is 1. Because the workers are forked after the first granule, they inherit the building and land-mask trees globato builds for the region instead of each rebuilding them.
- `ivert database download` fetches the ATL08 and ATL24 granules that classification needs ahead of it (#108), six at once, in a child process that runs alongside the classification, largest subset first. Each subset is classified once its files have arrived, so the classifiers never download for themselves and the prefetch's progress bars are the only ones on the screen, laid out one per download as in the ATL03 stage, rather than several processes' bars overwriting one another. globato used to fetch them one granule at a time as it went, and with the classification itself down to seconds the wait for a granule's two aux files (often a couple of hundred megabytes together) was most of its time. If the prefetch fails or the platform cannot fork, the classifiers fetch for themselves as before.
- A plain bounding box given to `ivert database download` is now requested from Harmony the same way a vector file's region is: covered with the fewest rectangles that fit it, each one request (#106). It used to be cut into 2° tiles first, so a typed box that matched one of a shapefile's rectangles produced different requests, different cached subsets and different Harmony jobs from the shapefile. One large subset of a granule also costs Harmony less than several small ones. The `split_big_bboxes` parameter of `download_new_granules()` is gone; its `tile_size_deg` and `max_tile_scale_factor` now describe storage instead: each downloaded subset is classified once and stored as one `.nc` file per tile of roughly 2°, as before, because a query reads every file that touches it in full and small files keep queries cheap. The squares a region is covered with are now 2° too, rather than the 1° of #105, so the storage tiles fall exactly on the request grid. `_process_h5_to_nc()` is now a thin wrapper over `_classify_h5()`, which runs globato once per granule, and `_write_nc()`, which writes one tile.
