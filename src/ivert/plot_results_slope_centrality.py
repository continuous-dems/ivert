...
####################################3
import ivert.utils.configfile  # noqa: E402

my_config = ivert.utils.configfile.Config()
import ivert.icesat2.plot_validation_results  # noqa: E402
...
def add_lat_lons(df):
    ...
    for idx, ((i, j), row) in enumerate(df.iterrows()):
        fname = row["filename"]
        resolution_s = float(re.search(r"(?<=_)\d{1,2}(?=s_)", fname).group())
        res_deg = resolution_s / (60 * 60)
        tile_lat_str = re.search(r"(?<=_)[NS]\d{2}(?=[EW]\d{3}_)", fname).group()
        tile_lon_str = re.search(r"(?<=_[NS]\d{2})[EW]\d{3}(?=_)", fname).group()
...
def get_slopes(df, files_dirname):
    ...
    fnames_dict = {
        os.path.basename(fn): sfn for fn, sfn in zip(fnames, slope_fnames, strict=True)
    }
...
def plot_errors_against_slope_centrality(
    results_h5_or_list,
    output_figure_name,
    ...
    if os.path.exists(total_results_h5):
        ...
        data = ivert.icesat2.plot_validation_results.get_data_from_h5_or_list(
            results_h5_or_list,
            empty_val=empty_val,
...
if __name__ == "__main__":
    dirname = (
        "/home/mmacferrin/Research/DATA/ETOPO/data/validation_results/15s/2022.09.29"
    )
    h5_list = sorted(
        [
    ...
    plot_errors_against_slope_centrality(
        h5_list,
        os.path.join(outdir, "coverage_results.png"),
        coverage_cutoff_pct=47,
        exclude_ice_sheets=True,
    )  # , fig_title=os.path.basename(h5_list[0]))
